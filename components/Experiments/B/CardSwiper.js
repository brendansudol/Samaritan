  ...
  initPanResponder = swipeThresholdDistanceBase => {
    const panResponder = PanResponder.create({
      ...
      onPanResponderMove: (event, gesture) => {
        this.swipeCardAnimatedPosition.setValue({
          x: gesture.dx,
          y: gesture.dy,
        })
      },
      onPanResponderRelease: (event, gesture) => {
        ...
        if (
          gesture.dx >
          swipeThresholdDistanceFactor * swipeThresholdDistanceBase
        ) {
  ...
  forceSwipe = (direction, swipeMethodProps) => {
    ...
    switch (direction) {
      case SwipeDirections.RIGHT: {
        targetPositionX = SCREEN_WIDTH + 25
        break
      }
